...
@borg.on(events.NewMessage(pattern=r"\.setpotify ?(.*)", outgoing=True))
async def set_biostgraph(event):
    ...
    save_json = r.json()
    to_create = {'bio': "test", 'access_token': save_json['access_token'], 'refresh_token': save_json['refresh_token'],
